...
/*  这样可以优化 createStore 成一个参数，因为 state 和 stateChanger 合并到一起了：
...
/*  createStore 内部第一次的 dispatch 导致 state 初始化完成，
/*  后续外部的 dispatch 就是修改数据的行为了。

/*  给 stateChanger 这个玩意起一个通用的名字：reducer，不要问为什么，
/*  它就是个名字而已，修改 createStore 的参数名字：/*
/*/
...
/*  这是一个最终形态的 createStore，它接受的参数叫 reducer，
...
/*  如果有传入 state 的话，就会根据 action 来“修改“数据， 但其实它没有、
/*  也规定不能修改 state，而是要通过上节所说的把修改路径的对象都复制一遍，
/*  然后产生一个新的对象返回。如果它不能识别你的 action，它就不会产生新的数据，
/*  而是（在 default 内部）把 state 原封不动地返回。

/*  reducer 是不允许有副作用的。你不能在里面操作 DOM，也不能发 Ajax 请求，
/*  更不能直接修改 state，它要做的仅仅是 —— 初始化和计算新的 state。
/*
/*  现在可以用这个 createStore 来构建不同的 store 了，只要给它传入符合上述的定义的 reducer 即可：
/*
/***************************************************************************/
/*  栗子：*/
/*
